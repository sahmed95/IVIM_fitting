...
class IVIM_Curve:
    # This class wraps up some IVIM curves and the minimize solver for easier IVIM curve fitting
    __author__ = 'eric'
...
def IVIM_array_fit(fit_class, x0, data, mask, other_params, bvals):
    ...
    itershape = data.shape[0:-1]  # crop the final dimension because that's the one that's fit
    if type(x0) is list:
        numparams = len(x0)
        x0_use = x0
    else:
        numparams = x0.shape[-1]
    fit = np.zeros(list(itershape)+[numparams, ])  # convert tuple to a list and append 4 to the end
    ...
    for item in np.ndindex(itershape):  # here's how to run a flat iterator!
        idx = np.ravel_multi_index(item, itershape)  # +1 would avoid printing 0 twice
        ...
        if (not any(data[item] == 0)) and (mask[item] > 0):
            ...
            if not type(x0) is list:
                x0_use = x0[item]
            res = fit_class.minimize(x0_use, (bvals, data[item], other_params_use))
            # print(res.x)
            fit[item] = res.x  # vector of [S0, f, D, D*]
            nit[item] = res.nit  # single value, number of iterations
            success[item] = res.success  # single value: True, False. if it minimized well
